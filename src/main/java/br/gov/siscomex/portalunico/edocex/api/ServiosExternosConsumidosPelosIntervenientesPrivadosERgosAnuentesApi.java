...
public interface ServiosExternosConsumidosPelosIntervenientesPrivadosERgosAnuentesApi  {
     ...
     *
     * Esse serviço somente poderá ser utilizado quando houver apenas um dossiê vinculado à operação e para os seguintes tipos de operações:&lt;ul&gt;&lt;li&gt;DI - Declaração de Importação&lt;br/&gt;&lt;/li&gt;&lt;li&gt;LI - Licença de Importação&lt;br/&gt;&lt;/li&gt;&lt;li&gt;RE - Registro de Exportação&lt;br/&gt;&lt;/li&gt;&lt;li&gt;DT - Declaração de Trânsito&lt;br/&gt;&lt;/li&gt;&lt;li&gt;DIR - Declaração de Importação de Remessa&lt;br/&gt;&lt;/li&gt;&lt;li&gt;DUE - Declaração Única de Exportação&lt;br/&gt;&lt;/li&gt;&lt;li&gt;CATP - Catálogo de Produtos&lt;br/&gt;&lt;/li&gt;&lt;li&gt;LPCO - Tratamento Administrativo/LPCO&lt;br/&gt;&lt;/li&gt;&lt;/ul&gt;
     *
     */
    @POST
    ...
    @Consumes({ "application/json" })
    @Produces({ "application/json" })
    @ApiOperation(value = "Anexar documento à operação.", notes = "Esse serviço somente poderá ser utilizado quando houver apenas um dossiê vinculado à operação e para os seguintes tipos de operações:<ul><li>DI - Declaração de Importação<br/></li><li>LI - Licença de Importação<br/></li><li>RE - Registro de Exportação<br/></li><li>DT - Declaração de Trânsito<br/></li><li>DIR - Declaração de Importação de Remessa<br/></li><li>DUE - Declaração Única de Exportação<br/></li><li>CATP - Catálogo de Produtos<br/></li><li>LPCO - Tratamento Administrativo/LPCO<br/></li></ul>", tags={ "Serviços externos consumidos pelos intervenientes privados e órgãos anuentes" })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "Operação realizada com sucesso", response = RetornoAnexarDocumento.class),
        @ApiResponse(code = 400, message = "Requisição mal formatada"),
        @ApiResponse(code = 401, message = "Usuário não autenticado ou autenticação inválida"),
    ...
    @ApiResponses(value = { 
        ...
        @ApiResponse(code = 422, message = "Erro(s) de validação da camada de negócio"),
        @ApiResponse(code = 500, message = "Erro interno no servidor") })
    public Response consultarDocumentoOperacao(@ApiParam(value = "Identifica o tipo de operação desejado:<br/>DI - Declaração de Importação<br/>LI - Licença de Importação<br/>RE - Registro de Exportação<br/>DT - Declaração de Trânsito<br/>DIR - Declaração de Importação de Remessa<br/>CATP - Catálogo de Produtos<br/>DUE - Declaração Única de Exportação<br/>DUIMP - Declaração Única de Importação,<br/>LPCO - Tratamento Administrativo/LPCO<br/>RECINTOS - Recintos Aduaneiros",required=true, allowableValues="DI, LI, RE, DT, DIR, CATP, DUE, DUIMP, LPCO, RECINTOS") @PathParam("tipoOperacao") String tipoOperacao, @ApiParam(value = "Número da operação.<br/>Tamanho máximo: 255<br/>Formato: de acordo com o definido em cada sistema de origem do tipo de operação.",required=true) @PathParam("numeroOperacao") String numeroOperacao, @ApiParam(value = "JSON Web Token (JWT) contendo as informações do usuário. Recuperado no parâmetro Set-Token no response da autenticação." ,required=true)@HeaderParam("Authorization") String authorization, @ApiParam(value = "Token de prevenção contra ataques CSRF. Recuperado no parâmetro X-CSRF-Token no response da autenticação." ,required=true)@HeaderParam("X-CSRF-Token") String xCSRFToken);
