...
public interface ServiosDeEntregaDaCargaApi {
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(code = 422, message = "Erro(s) de validação da camada de negócio"),
            @ApiResponse(code = 500, message = "Erro interno no servidor")})
    Response impedimentosEntregaImportadorUsingGET(@ApiParam(value = "JSON Web Token (JWT) contendo as informações do usuário. Este token é recuperado no parâmetro Set-Token no response da autenticação", required = true) @HeaderParam("Authorization") String authorization, @ApiParam(value = "Token de prevenção contra ataques CSRF. Este token é recuperado no parâmetro X-CSRF-Token no response da autenticação", required = true) @HeaderParam("X-CSRF-Token") String xCSRFToken, @NotNull @ApiParam(value = "Identificação da carga (Conhecimento/DSIC)<br/>Tamanho mínimo: 1<br/>Tamanho máximo: 35", required = true) @QueryParam("identificacaoCarga") String identificacaoCarga, @NotNull @ApiParam(value = "Número de documento de saída<br/>Tamanho mínimo: 1<br/>Tamanho máximo:17<br/> Formato: NNNNNNNNNNNNNNNNN", required = true) @QueryParam("numeroDocumentoSaida") String numeroDocumentoSaida, @NotNull @ApiParam(value = "Tipo de carga. <p>Os tipos podem ser:</p><ul><li><em>740</em> – Air Waybill.</li><li><em>741</em> – Master Air Waybill.</li><li><em>703</em> – House Air Waybill.</li><li><em>14</em> – DSIC - Documento Subsidiário de Identificação de Carga.</li></ul>", required = true, allowableValues = "740, 741, 703, 14") @QueryParam("tipoCarga") String tipoCarga, @NotNull @ApiParam(value = "Tipo de documento de saída", required = true, allowableValues = "DI, PROCESSO, EDMOV, OUTROS, PMD, PDE_TR, DSI, DSI_FORMULARIO, DUIMP") @QueryParam("tipoDocumentoSaida") String tipoDocumentoSaida, @ApiParam(value = "CNPJ do responsável pelo arquivo do conhecimento de carga.<br/>Somente deve ser informado para conhecimentos do tipo HAWB<br/>Tamanho: 8<br/>Formato: NNNNNNNN ou <br/>Tamanho: 14<br/>Formato: NNNNNNNNNNNNNN") @QueryParam("cnpjResponsavelArquivo") String cnpjResponsavelArquivo, @ApiParam(value = "Data da emissão do conhecimento.<br/>Obrigatório para tipos de conhecimento AWB, MAWB e HAWB.<br/>Para o tipo DSIC não deve ser informado.<br/>Formato: yyyy-MM-dd") @QueryParam("dataEmissao") String dataEmissao);
}
