...
import br.gov.siscomex.portalunico.ccta.model.CargaRodoviariaDetalhada;
import br.gov.siscomex.portalunico.ccta.model.CargaRodoviariaFluxoLogistico;
...
public interface ServicosDeConsultaDeConhecimentoDeCargaRodoviariaApi {
    ...
    CargaRodoviariaDetalhada listarCargaDetalhadaUsingGET(@ApiParam(value = "JSON Web Token (JWT) contendo as informações do usuário. Este token é recuperado no parâmetro Set-Token no response da autenticação", required = true) @HeaderParam("Authorization") String authorization, @ApiParam(value = "Token de prevenção contra ataques CSRF. Este token é recuperado no parâmetro X-CSRF-Token no response da autenticação", required = true) @HeaderParam("X-CSRF-Token") String xCSRFToken, @NotNull @ApiParam(value = "Data da emissão do conhecimento  Tamanho: 10  Formato YYYY-MM-DD", required = true) @QueryParam("dataEmissao") String dataEmissao, @NotNull @ApiParam(value = "Número do Conhecimento", required = true) @QueryParam("numeroConhecimento") String numeroConhecimento, @ApiParam(value = "CNPJ Responsável  Tamanho: 14  Formato: NNNNNNNNNNNNNN") @QueryParam("cnpjResponsavel") String cnpjResponsavel);

    /**
     * Retorna o conhecimento de carga do modal rodoviário do Fluxo Logístico
     *
     * Retorna o conhecimento de carga do modal rodoviário do Fluxo Logístico
     *
     */
    @GET
    @Path("/api/ext/rodoviario/cargas/fluxo-logistico")
    @Produces({"application/json"})
    @ApiOperation(value = "Retorna o conhecimento de carga do modal rodoviário do Fluxo Logístico", notes = "Retorna o conhecimento de carga do modal rodoviário do Fluxo Logístico", tags = {"Servicos de Consulta de Conhecimento de Carga Rodoviaria"})
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "Operação realizada com sucesso", response = CargaRodoviariaFluxoLogistico.class),
            @ApiResponse(code = 204, message = "Operação realizada com sucesso. Nenhum conteúdo retornado"),
            @ApiResponse(code = 400, message = "Requisição mal formatada"),
            @ApiResponse(code = 401, message = "Usuário não autenticado ou autenticação inválida"),
            @ApiResponse(code = 403, message = "Usuário não tem permissão de acesso ao recurso"),
            ...
            @ApiResponse(code = 422, message = "Erro(s) de validação da camada de negócio"),
            @ApiResponse(code = 500, message = "Erro interno no servidor")})
    CargaRodoviariaFluxoLogistico listarCargaFluxoLogisticoUsingGET(@ApiParam(value = "JSON Web Token (JWT) contendo as informações do usuário. Este token é recuperado no parâmetro Set-Token no response da autenticação", required = true) @HeaderParam("Authorization") String authorization, @ApiParam(value = "Token de prevenção contra ataques CSRF. Este token é recuperado no parâmetro X-CSRF-Token no response da autenticação", required = true) @HeaderParam("X-CSRF-Token") String xCSRFToken, @NotNull @ApiParam(value = "Data da emissão do conhecimento  Tamanho: 10  Formato YYYY-MM-DD", required = true) @QueryParam("dataEmissao") String dataEmissao, @NotNull @ApiParam(value = "Número do Conhecimento", required = true) @QueryParam("numeroConhecimento") String numeroConhecimento, @ApiParam(value = "CNPJ Responsável  Tamanho: 14  Formato: NNNNNNNNNNNNNN") @QueryParam("cnpjResponsavel") String cnpjResponsavel);
}
