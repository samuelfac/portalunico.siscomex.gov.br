...
import br.gov.siscomex.portalunico.cada.model.ChangeLogArquivoDiarioAtributoNcmDTO;
...
public interface RelacaoDeAtributosPorNcmApi {
    ...
    @ApiOperation(value = "Consulta da relação de atributos de um código NCM", notes = "", tags = {"Relacao de Atributos por NCM"})
    @ApiResponses(value = {
    ...
    @ApiOperation(value = "Download do arquivo com a relação de atributos por NCM", notes = "", tags = {"Relacao de Atributos por NCM"})
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "Operação realizada com sucesso"),
            @ApiResponse(code = 400, message = "Requisição inválida"),
            @ApiResponse(code = 422, message = "Erro(s) de validação da camada de negócio"),
    ...
    @ApiOperation(value = "Consulta histórico de versões", notes = "", tags = {"Relacao de Atributos por NCM"})
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "Operação realizada com sucesso", response = ChangeLogArquivoDiarioAtributoNcmDTO.class, responseContainer = "List"),
            @ApiResponse(code = 400, message = "Requisição inválida"),
            @ApiResponse(code = 422, message = "Erro(s) de validação da camada de negócio"),
            @ApiResponse(code = 401, message = "Usuário não autenticado ou autenticação inválida"),
            @ApiResponse(code = 500, message = "Erro interno no servidor"),
            ...
            @ApiResponse(code = 403, message = "Usuário não tem permissão de acesso ao recurso"),
            @ApiResponse(code = 404, message = "Recurso não encontrado")})
    List<ChangeLogArquivoDiarioAtributoNcmDTO> obterHistoricoVersoesArquivoDiario(@ApiParam(value = "JSON Web Token (JWT) contendo as informações do usuário. Este token é recuperado no parâmetro Set-Token no response da autenticação", required = true) @HeaderParam("Authorization") String authorization, @ApiParam(value = "Token de prevenção contra ataques CSRF. Este token é recuperado no parâmetro X-CSRF-Token no response da autenticação", required = true) @HeaderParam("X-CSRF-Token") String xCSRFToken, @ApiParam(value = "Lista de números de versões a ser recuperado") @QueryParam("versao") List<Integer> versao);
}
