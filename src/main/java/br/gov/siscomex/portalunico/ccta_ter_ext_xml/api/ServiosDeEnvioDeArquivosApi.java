package br.gov.siscomex.portalunico.ccta_ter_ext_xml.api;

import br.gov.siscomex.portalunico.ccta_ter_ext_xml.model.ManifestoCrt;
import br.gov.siscomex.portalunico.ccta_ter_ext_xml.model.ManifestoViagem;
import br.gov.siscomex.portalunico.ccta_ter_ext_xml.model.RetornoManifestoCrt;
import br.gov.siscomex.portalunico.ccta_ter_ext_xml.model.RetornoManifestoViagem;
...
public interface ServiosDeEnvioDeArquivosApi {
    ...
    @ApiOperation(value = "Informar Manifesto do CRT", notes = "<p style=\"margin-bottom: 1em; margin-top: 1em;\"> Esse serviço tem por objetivo possibilitar a manifestação do CRT pelo transportador nacional ou estrangeiro (este através de um representante CNPJ ou CPF)</p><p style=\"margin-bottom: 1em; margin-top: 1em;\"> O processamento dessa manifestação é realizado de forma assíncrona, sendo retornado apenas o número de protocolo para consulta posterior.</p>", tags = {"Serviços de Envio de Arquivos"})
    @ApiResponses(value = {
    ...
    Response doPostUsingPOST(@ApiParam(value = "", required = true) @Valid ManifestoCrt body, @ApiParam(value = "JSON Web Token (JWT) contendo as informações do usuário. Este token é recuperado no parâmetro Set-Token no response da autenticação", required = true) @HeaderParam("Authorization") String authorization, @ApiParam(value = "Token de prevenção contra ataques CSRF. Este token é recuperado no parâmetro X-CSRF-Token no response da autenticação", required = true) @HeaderParam("X-CSRF-Token") String xCSRFToken);
    ...
    @Consumes({"application/json"})
    @Produces({"application/json"})
    @ApiOperation(value = "Informar Manifesto do Viagem", notes = "<p style=\"margin-bottom: 1em; margin-top: 1em;\"> Esse serviço tem por objetivo possibilitar a entrega da carga para o destinatário final,  através do conhecimento de carga/DSIC e do documento de saída vinculado ao mesmo. A entrega poderá ser realizada de forma total ou parcial.</p><p style=\"margin-bottom: 1em; margin-top: 1em;\"> O processamento dessa entrega é realizado de forma assíncrona, sendo retornado apenas o número de protocolo para consulta posterior.</p>", tags = {"Serviços de Envio de Arquivos"})
    @ApiResponses(value = {
