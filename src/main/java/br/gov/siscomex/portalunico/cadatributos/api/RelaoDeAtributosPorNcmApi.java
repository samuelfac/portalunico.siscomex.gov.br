...
import br.gov.siscomex.portalunico.cadatributos.model.ChangeLogArquivoDiarioAtributoNcmDTO;
...
public interface RelaoDeAtributosPorNcmApi {
    ...
    @ApiOperation(value = "Consulta histórico de versões", notes = "", tags = {"Relação de Atributos por NCM"})
    @ApiResponses(value = {
            ...
            @ApiResponse(code = 403, message = "Usuário não tem permissão de acesso ao recurso"),
            @ApiResponse(code = 404, message = "Recurso não encontrado")})
    Response obterHistoricoVersoesArquivoDiario(@ApiParam(value = "JSON Web Token (JWT) contendo as informações do usuário. Este token é recuperado no parâmetro Set-Token no response da autenticação", required = true) @HeaderParam("Authorization") String authorization, @ApiParam(value = "Token de prevenção contra ataques CSRF. Este token é recuperado no parâmetro X-CSRF-Token no response da autenticação", required = true) @HeaderParam("X-CSRF-Token") String xCSRFToken, @ApiParam(value = "Lista de números de versões a ser recuperado") @QueryParam("versao") List<Integer> versao);
}
